...
- an **MCP server** so a clever wizard like Claude Code can search your library.
...
> Made with **vibe coding** — Gobby was written hand-in-hand with Claude, then
> read over line by line. A free elf and a clever wizard, oh yes.
...
Gobby exposes an MCP server at `/mcp` on the same port. Add it and Claude can use:
