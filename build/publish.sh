...
# in each filename (gobby-<platform>[.exe]). Pure-Go SQLite (modernc) + ffmpeg
# embedded as WebAssembly (go-ffmpreg) means no CGO, no toolchain per target, and
# a single self-contained binary — nothing to download. Runs from anywhere.
...
for t in "${targets[@]}"; do
  ...
  CGO_ENABLED=0 GOOS=$goos GOARCH=$goarch \
    go build -trimpath -tags embedffmpeg -ldflags "$LDFLAGS" -o "$OUT/$name" ./src
done
